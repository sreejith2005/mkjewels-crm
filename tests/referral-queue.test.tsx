...
describe("ReferralQueue legacy parity", () => {
  ...
  it("uses referral-specific Today semantics and excludes overdue referrals", () => { renderQueue([{ ...item, next_followup_date: "2000-01-01" }]); expect(screen.getByText("No referrals match this view.")).toBeTruthy(); });
  it("uses the legacy form field order and saves through the parity RPC", async () => { rpc.mockResolvedValue({ error: null }); renderQueue(); fireEvent.click(screen.getByRole("button", { name: "FOLLOW UP FORM" })); for (const label of ["Follow Up Status", "Call Response", "Next Follow Up Date", "Entered By", "Follow Up Remark"]) expect(screen.getByLabelText(label)).toBeTruthy(); expect(screen.getByText("Action Point:")).toBeTruthy(); fireEvent.change(screen.getByLabelText("Follow Up Remark"), { target: { value: "Called client" } }); fireEvent.click(screen.getByRole("button", { name: "Save" })); await waitFor(() => expect(rpc).toHaveBeenCalledWith("save_referral_followup", expect.objectContaining({ p_referral_calling_id: "referral-1", p_followup_status: "PENDING", p_call_response: "CONNECTED", p_entered_by: "Test CRM" }))); });
