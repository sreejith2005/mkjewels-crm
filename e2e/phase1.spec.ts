...
test.describe("Phase 1 client CRM", () => {
  ...
  test("phone search resolves exact and partial matches", async ({ page }) => { const search = page.getByLabel("Search client"); await search.fill(process.env.E2E_CLIENT_PHONE!); await expect(page.getByText(process.env.E2E_CLIENT_NAME!)).toBeVisible(); await search.fill(process.env.E2E_CLIENT_PHONE!.slice(-4)); await expect(page.getByText(process.env.E2E_CLIENT_NAME!)).toBeVisible(); });
