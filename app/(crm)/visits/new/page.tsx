...
export default async function NewVisitPage({ searchParams }: { searchParams: Promise<{ queue?: string }> }) {
  ...
  const [{ data: allocation }, { data: availability }, { data: productCategories }, { data: notBoughtReasons }, { data: beverages }, { data: snacks }] = await Promise.all([branchId ? supabase.from("crm_allocation").select("crm_name").eq("branch_id", branchId).eq("active", true).order("crm_name") : Promise.resolve({ data: [] }), branchId ? supabase.from("crm_daily_availability").select("crm_name,is_available").eq("branch_id", branchId).eq("date", today) : Promise.resolve({ data: [] }), supabase.from("lookup_product_categories").select("label").eq("active", true).order("label"), supabase.from("lookup_not_bought_reasons").select("label").eq("active", true).order("label"), supabase.from("lookup_beverages").select("label").eq("active", true).order("label"), supabase.from("lookup_snacks").select("label").eq("active", true).order("label")]);
  return <main className="mx-auto max-w-7xl px-5 py-7"><p className="text-sm font-semibold uppercase tracking-wider text-amber-800">Visit intake</p><h1 className="mt-1 text-3xl font-semibold">Walk-in visit form</h1><p className="mt-2 text-stone-600">Nothing is saved until the completed visit is submitted.</p><WalkInForm profile={{ role: profile.role, branchId: user?.branch_id ?? null, name: user?.name ?? "" }} branches={branches ?? []} crms={availableCrmNames(allocation ?? [], availability ?? [])} queue={queueResult.data} client={client} lookups={{ productCategories: (productCategories ?? []).map((item) => item.label), notBoughtReasons: (notBoughtReasons ?? []).map((item) => item.label), beverages: (beverages ?? []).map((item) => item.label), snacks: (snacks ?? []).map((item) => item.label) }} /></main>;
}
